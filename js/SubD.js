...
var p = new Float32Array(3*65000);
var p1 = new Float32Array(3*65000);
var norm = new Float32Array(3*65000);
...
function subMesh (gl, path, level)
{
  ...
  var cp = mesh.vertices;
  var faces = mesh.faces;

   cp = [0,0,2, 2,0,-.66, -1,1.7,-.67, -1,-1.7,-.67]; // Control Points
   faces = [0,1,2, 0,2,3, 0,3,1, 3,2,1];

  // cp = [-0.500000, -0.500000, 0.500000,  0.500000, -0.500000, 0.500000, -0.500000, 0.500000, 0.500000,  0.500000, 0.500000, 0.500000,  -0.500000, 0.500000, -0.500000,  0.500000, 0.500000, -0.500000,  -0.500000, -0.500000, -0.500000,   0.500000, -0.500000, -0.50000]

  // faces= [0,1,2, 2,1,3, 2,3,4, 4,3,5, 4,5,6, 6,5,7, 6,7,0, 0,7,1, 1,7,3, 3,7,5, 6,0,4, 4,0,2];
  ...
  var n = level;
   ...
   // parser
   var ce = [], vior = []; // Control Edges
     // vior [[,],[,],...] - ordered list of neighbor edges [ed_id, nb vert_id (0 or 1)] of a vertex
   ei = [];  fi = [];  vi = [];  // indexed edges, faces and vertices
   ...
   for (var it = 0; it < n; it++){
    ...
    vil = vi.length;
    for (var i = 0; i < vil; i++){
     var sx = sy = sz = 0,  lv = vi[i].length;
     for (var k = 0; k < lv; k++){
       var j = 3* ei[vi[i][k][0]].PS[vi[i][k][1]];
       sx += p[j++];  sy += p[j++];  sz += p[j++];}
     var j = 3*i;
     p1[j] = an[lv]*p[j] + sx*(1 - an[lv])/lv;  j++;
     p1[j] = an[lv]*p[j] + sy*(1 - an[lv])/lv;  j++;
   ...
   for (var i = 0; i < vil; i++){
    ...
    for (var k = 0; k < lv; k++){
      var j = 3*ei[vi[i][k][0]].PS[vi[i][k][1]];
      x1 += tauc[lv][k]*p[j];  x2 += taus[lv][k]*p[j];
      sx += p[j++];
      y1 += tauc[lv][k]*p[j];  y2 += taus[lv][k]*p[j];
      sy += p[j++];
      z1 += tauc[lv][k]*p[j];  z2 += taus[lv][k]*p[j];
      sz += p[j++];
    }
    ...
    var j = 3*i;
    norm[j] = x3/norm3;
    p1[j] = bn[lv]*p[j] + sx*(1 - bn[lv])/lv;  j++;
    norm[j] = y3/norm3;
    p1[j] = bn[lv]*p[j] + sy*(1 - bn[lv])/lv;  j++;
    norm[j] = z3/norm3;
    p1[j] = bn[lv]*p[j] + sz*(1 - bn[lv])/lv;
   }  

  var vil3 = 3*vil;
  for (var i = 0; i < vil3; i++) p[i] = p1[i];
  for (var i = 0; i < cpl3; i++) p[vil3 + i] = cp[i];

    var ind = [];
   fil = fi.length;  fil3 = 3*fil;
   for (var i = 0; i < fil; i++) ind.push (fi[i].P0, fi[i].P1, fi[i].P2);
   off = 2*ind.length;
   for (var i = 0; i < ce.length; i++) ind.push (ce[i][0] + vil, ce[i][1] + vil);
  ...
  model.addAttrib("position", p1);
  model.addAttrib("normal", norm);
  //model.addAttrib("texCoord", mesh.textures, 2);
  model.setIndices(ind);
